...
selectedData <- reactive({
  data <- brushedPoints(dados_completos, input$plot_brush)
  if (nrow(data) == 0)
    data <- dados_completos
  data
})

renderPlot({ggplot(dados_completos, aes(x = Q2, y = Q7, color = Q1)) +
  ...
  theme_minimal() +
  theme(legend.position = "bottom")}, outputArgs = list(brush = brushOpts(id = "plot_brush")))
```
...
```{r, echo=FALSE}
renderPlot({
  ggplot(selectedData(), aes(x=Q4,fill=Q4)) +
  geom_bar() +
